...
# --- Configuration ---
CONFIG = {
    "model_path": "YOUR/LOCAL/LLAMA3/PATH",
    "input_file": "patch_db.json",
    "output_file": "llama_agent_metrics_final.txt",
    "max_new_tokens": 512,
    "temperature": 0.1,
    "context_window": 8192,
}
...
print("Loading local Llama-3.1-8B-Instruct model...")
...
# --- Tool Definitions (adapted for Llama-3 chat format) ---
...
3. "Unclear" if uncertain<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
...
You MUST respond with either "Yes" or "No".<|eot_id|><|start_header_id|>assistant<|end_header_id|>"""
...
# --- System Prompt (to be manually included in user prompt) ---
SYSTEM_PROMPT_CONTENT = """You are an expert software security analyst. Your task is to determine whether a given code commit is a security patch.
...
# --- Prompt Builder: embed system instructions into user message using Llama-3 format ---
def build_react_prompt(row):
    diff_code = str(row['diff_code'])
    commit_message = str(row['commit_message'])

    user_content = f"""{SYSTEM_PROMPT_CONTENT}
